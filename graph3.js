...
/* Link naar de dataset http://statline.cbs.nl/Statweb/publication/?DM=SLNL&PA=80785ned&D1=2&D2=a&D3=30&D4=a&HDR=T&STB=G2%2cG1%2cG3&CHARTTYPE=1&VW=T */
...
var cities = data.columns.slice(1).map(function(id) {
    return {
      ...
      values: data.map(function(d) {
        return {datum: d.datum, aantal: d[id]};
      })
    };
});
...
y.domain([
    d3.min(cities, function(c) {
        return d3.min(c.values, function(d) { return d.aantal; }); 
    }),
    d3.max(cities, function(c) { 
        return d3.max(c.values, function(d) { return d.aantal; }); 
    })
    ]);
...
g.append("g")
    ...
    .attr("fill", "#000")
    .text("Aantal cultuurgrond in ha");
...
klasse.append("text")
    ...
    .attr("transform", function(d) { return "translate(" + x(d.value.datum) + "," + y(d.value.aantal) + ")"; })
    .attr("x", 3)
    .attr("dy", "0.35em")
